...
EFI_GUID ymhFileInfoGuid = EFI_FILE_INFO_ID;

EFI_SYSTEM_TABLE *ymhST = NULL;
...
CHAR16 *ymhLogFile = (CHAR16 *)L"ymhuefidbg.log";
...
EFI_STATUS ymhInitLogFile(CHAR16 *s)
{
    ...
    //move to end of file
    status = ymhFileHandle->GetInfo(ymhFileHandle, &ymhFileInfoGuid, &size, NULL);
    size += 2;
    ymhST->BootServices->AllocatePool(EfiRuntimeServicesData,
                                      size,
                                      &fileInfo);
...
void __cdecl ymhPort80(UINT8 num)
{
    _ReadWriteBarrier();
    _outp(0x80, num);
    _ReadWriteBarrier();
